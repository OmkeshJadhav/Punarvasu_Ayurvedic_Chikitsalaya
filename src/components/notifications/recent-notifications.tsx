 ...
 *
 * ## One panel, two dashboards
 *
 * The patient's home and the practitioner's day both show the same thing — the
 * last few notifications, newest first, each linking to the resource it is
 * about. So it is one component taking its words as a parameter rather than
 * two components that would drift apart, which is `AGENTS.md` section 32's
 * rule and the reason `RecentUpdates` is now a wrapper around this.
 *
 * Only the empty and failure sentences differ, because only they say anything
 * about the reader. The notification rows themselves were written by
 * `templates.ts` when they were created and are rendered here unchanged.
 *
 ...
 * title — and, for a practitioner, no patient name. There is no column on
 * `notifications` for any of them. So section 39's "minimise sensitive
 * content" is satisfied by the schema rather than by truncation here, and this
 * panel can show the real title safely.
 *
 ...
 */
export interface RecentNotificationsCopy {
  /** Shown when the read failed — not when there is nothing. */
...
export function RecentNotifications({
  ...
  readonly copy: RecentNotificationsCopy;
}) {

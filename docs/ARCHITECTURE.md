...
| Notifications | **Implemented (Phase 15).** `features/notifications/` holds the domain model, the templates, the link builder, the retry policy, the error and provider-failure mappers, the query layer, four server actions, the processor and the dispatch seam; `lib/notifications/` holds the channel abstraction and the EmailJS adapter; `components/notifications/` holds the bell (a server component feeding a hover/tap preview panel of the newest five), the list, the filters, the preference grid and the dashboard glance. **The event is a row a database trigger writes inside the domain transaction, not a call the application makes** — so a provider outage cannot affect a booking, and no write path can forget to emit. Nothing takes a recipient or a link: both are derived from the **audience** and the resource inside `create_notification`. Two audiences: a **patient** is told about their care, and a **practitioner** about changes to their own diary made by somebody else — one domain event, two notification rows, no second trigger. A practitioner message names no patient, because the template has no field for one. In-app works today; **email exists as an adapter and is not configured, so the channel is disabled and says so**; there is no SMS and no WhatsApp anywhere. Both audiences are applied and verified live |
...
| Recipient resolution | `public.notification_recipient_for_resource()` — takes an **audience** and a **resource**, never a person. `patients.profile_id` for a patient, `practitioners.profile_id` for a practitioner |
| Audiences | `notification_audience` — `patient` and `practitioner`. One domain event fans out to both in the processor; **no second trigger and no second outbox row**. Each audience carries its own idempotency key |
...
| Templates | `features/notifications/templates.ts`, typed per event and per audience, versioned. A practitioner's template has **no field for a patient's name**, so a practitioner message cannot name one |
...
| The notification centre | `/notifications`, `/notifications/preferences`, `components/notifications/`. One page for every role; the **words** differ by audience and nothing else does. `RecentNotifications` is the glance, shared by `/patient` and `/doctor` |

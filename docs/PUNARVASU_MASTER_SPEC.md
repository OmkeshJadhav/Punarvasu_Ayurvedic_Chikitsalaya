...
**A practitioner is told too** — when an appointment in their own diary is
confirmed, moved or cancelled, which are the changes the front desk makes
without them. One domain event becomes two notifications, decided by the
worker rather than by a second trigger, and a practitioner's message **names no
patient**: the template that writes it has no field for one, because a
notification reaches a lock screen and the diary behind the link is where
knowing who belongs. A practitioner is sent no reminder and nothing about the
prescriptions and plans they wrote themselves. A receptionist and an
administrator receive nothing at all, because no workflow has been designed for
either. *(Applied to the live
project and verified against it on 2026-09-23 —
`docs/progress/progress_phase_15.md` §20.7.)*

**Nothing takes a recipient or a link**: both are derived from the audience and
the resource inside the database, so there is no `recipientEmail` or
`recipientUserId` to substitute anywhere in the system, and no endpoint by
which one person can cause a message to reach another. A notification carries a title, a short body

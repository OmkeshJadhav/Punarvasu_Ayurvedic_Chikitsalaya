...
| **There is no recipient parameter anywhere** | `create_notification` does not take one: it resolves the account from the resource the notification is about, through `patients.profile_id`. There is no `recipientUserId`, no `recipientEmail`, no `recipientPhone` and no `to` in any function signature, any schema or any form in the feature — §110 and example 9 satisfied structurally rather than by filtering. Asserted by a scan over every function's parameter list in the migration. |
...
| **The link is derived, never accepted** | §19. `create_notification` builds the deep link from the resource type and id through `public.notification_link_path()`; there is no link parameter. A check constraint refuses anything that is not an application-relative path under a known area, with no scheme, no protocol-relative form and no traversal — the same refusal Phase 06's `safeRedirectPath` makes. |
...
| Preference isolation | `user_id = (select auth.uid())`, and `set_notification_preference` takes **no user id** — §97's `{"userId": "another-user"}` has nowhere to arrive, and §98's staff accounts are scoped by the same mechanism. |
...
| Own notifications **(implemented)** | Read own, mark read, set own preferences | Same — their own | Same — their own | Same — their own |
...
| Notifications | Confidential | The system | `notifications`, `notification_deliveries` | The recipient | Subject, heading and a link, to the email provider — never clinical content |
